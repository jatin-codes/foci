 ...
 */
/**
 * `hold` lets a test keep one request in flight: pass a promise and every write
 * waits on it, so the pending state can be observed before the request lands.
 */
export function fakeApi(initial: Todo[] = [], hold?: Promise<void>) {
  let todos = [...initial];
